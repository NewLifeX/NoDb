    ...
    public Boolean CheckCapacity(Int64 capacity)
    {
        ...
        lock (SyncRoot)
        {
            ...
            else
            {
                ...
                if (Readonly)
                {
                    fs ??= new FileStream(FileName.GetFullPath(), FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 4096, FileOptions.RandomAccess);
                    if (fs.Length < capacity) throw new InvalidDataException($"文件长度不足 {capacity}");
                }
                else
                {
                    fs ??= new FileStream(FileName.GetFullPath(), FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite, 4096, FileOptions.RandomAccess);
